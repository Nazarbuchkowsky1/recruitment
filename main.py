...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🧾 Вакансии":
        ...
        first_stage_text = """1️⃣ <b>Выбор вакансии</b>

• Перейдите во вкладку «Вакансии» и выберите подходящую позицию.

• Обратите внимание на требования к уровню языка, а также условия предоставления жилья (если указано работодателем)."""
...
Наша работа основана на официальных договорах и разрешениях, выданных уполномоченными структурами Германии и стран-партнёров. Все процедуры по подбору персонала, оформлению визовых и трудовых документов проходят в соответствии с положениями миграционного и трудового законодательства.

Каждый кандидат получает:
• Официальный трудовой договор от работодателя;
• Контрактовую поддержку;
...
Мы осознаём ответственность перед каждым соискателем и работаем на основании принципов законности, открытости и доверия, что делает нас надёжным, официально признанным партнером в вопросах международного трудоустройства.
...
💬 <b>Важно:</b> если по выбранной вакансии потребуются дополнительные документы, менеджер обязательно озвучит это при телефонном звонке."""
        ...
        second_stage_text = """2️⃣ <b>Заполнение заявки</b>

• Нажмите кнопку «Оставить заявку».

• Заполните все поля анкеты — неполные заявки не передаются менеджеру.
...
• После заполнения нажмите «Отправить»."""
        ...
        third_stage_text = """3️⃣ <b>Ожидание связи</b>

• Менеджер свяжется с вами в течение суток (иногда немного дольше — при большом количестве заявок).

• Важно указывать актуальные номера WhatsApp и Telegram, так как менеджер связывается именно через эти приложения."""
        ...
        fourth_stage_text = """4️⃣ <b>Первичная связь и собеседование с менеджером</b>

• Менеджер перезвонит, проведёт краткое интервью, уточнит вопросы работодателя и подробно проконсультирует по выбранной вакансии.

• Если вам всё подходит, менеджер выдаёт анкету соискателя, которую необходимо корректно и внимательно заполнить.

<b>Важно!</b>

Анкета — это первый этап вашей оценки.

Её рассматривают специалисты, чтобы принять решение: приглашать вас на собеседование или отклонять кандидатуру.

Если анкета набрала достаточное количество баллов, вас допускают к следующему этапу."""
        ...
        fifth_stage_text = """5️⃣ <b>Основное собеседование</b>

• Менеджер связывается с вами для согласования даты и времени собеседования.

• Собеседование проводится работодателем или его представителями.

• Решение о том, подходите ли вы на вакансию, обычно занимает несколько дней."""
        ...
        sixth_stage_text = """6️⃣ <b>Подписание договора и оформление документов</b>

• Если вы успешно проходите собеседование, менеджер ожидает договор между вами и визовым специалистом.

• На основании этого договора начинается подготовка рабочего контракта.

• После получения контракта вы отправляете его визовому специалисту — специалист формирует пакет документов для оформления визы в выбранную страну."""
        ...
        seventh_stage_text = """7️⃣ <b>Получение визы и финальные шаги</b>

• После получения визы вы уведомляете менеджера.

• Вместе обсуждаете организационные моменты, включая перелёт и дату выезда."""
